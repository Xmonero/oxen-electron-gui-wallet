<template>
  <div class="masternode-stake-tab">
    <div class="q-pa-md">
      <div class="q-pb-sm header">
        <span v-if="masternodes.length">
...
import ServiceNodeDetails from "./masternode_details";
import ServiceNodeList from "./masternode_list";
...
export default {
  ...
  computed: mapState({
    ...
    masternodes(state) {
      let nodes = state.gateway.daemon.masternodes.nodes;
    ...
    fetching: state => state.gateway.daemon.masternodes.fetching
  }),
  validations: {
  ...
  methods: {
    ...
    unlockWarning(node, event) {
      const key = node.masternode_pubkey;
      // stop detail page from popping up
      this.$gateway.send("wallet", "update_masternode_list");
