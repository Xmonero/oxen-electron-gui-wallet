<template>
  <div class="masternode-staking">
    <div class="q-px-md q-pt-md">
      <p class="tab-desc">
        {{ $t("strings.serviceNodeContributionDescription") }}
        <span
          style="cursor: pointer; text-decoration: underline;"
          @click="queneroWebsite"
          >Quenero {{ $t("strings.website") }}.</span
        >
      </p>
      <QueneroField
        :label="$t('fieldLabels.serviceNodeKey')"
        :error="$v.masternode.key.$error"
      >
        <q-input
          v-model.trim="masternode.key"
          :dark="theme == 'dark'"
      ...
      <QueneroField
        ...
        class="q-mt-md"
        :error="$v.masternode.amount.$error"
      >
        <q-input
          v-model.trim="masternode.amount"
          :dark="theme == 'dark'"
          type="number"
          min="0"
        ...
        <q-btn
          ...
          @click="masternode.amount = minStake(masternode.key)"
        />
        <q-btn
          color="primary"
          :text-color="theme == 'dark' ? 'white' : 'dark'"
          :label="$t('buttons.max')"
          :disable="!areButtonsEnabled()"
          @click="masternode.amount = maxStake(masternode.key)"
        />
      </QueneroField>
    ...
    <ServiceNodeContribute
      :awaiting-masternodes="awaiting_masternodes"
      class="contribute"
...
import { masternode_key, greater_than_zero } from "src/validators/common";
...
import ServiceNodeContribute from "./masternode_contribute";
import ServiceNodeMixin from "src/mixins/masternode_mixin";
...
export default {
  ...
  computed: mapState({
    ...
    stake_status: state => state.gateway.masternode_status.stake,
    ...
    awaiting_masternodes(state) {
      const nodes = state.gateway.daemon.masternodes.nodes;
  ...
  methods: {
    queneroWebsite() {
      const url = "https://quenero.tech/";
      this.$gateway.send("core", "open_url", {
        url
      });
    },
    fillStakingFields(key, minContribution) {
      this.masternode.key = key;
      this.masternode.amount = minContribution;
    },
    minStake() {
    ...
    maxStake() {
      const node = this.getNodeWithPubKey();
      return this.openForContriubtionQuenero(node);
    },
    getFeeDecimal(node) {
    ...
    getNodeWithPubKey() {
      const key = this.masternode.key;
      const nodeOfKey = this.awaiting_masternodes.find(
        n => n.masternode_pubkey === key
      );
      if (!nodeOfKey) {
        this.$q.notify({
          type: "negative",
    ...
    areButtonsEnabled() {
      // if we can find the masternode key in the list of masternodes
      const key = this.masternode.key;
      return !!this.awaiting_masternodes.find(
        n => n.masternode_pubkey === key
      );
    },
    async sweepAll() {
    ...
    async stake() {
      ...
      passwordDialog
        .onOk(password => {
          ...
          this.$gateway.send("wallet", "stake", masternode);
        })
        .onDismiss(() => {})
        .onCancel(() => {});
    }
