    ...
    <q-list class="masternode-list" no-border>
      ...
      >
          ...
          <q-item-label class="non-selectable">
            <span v-if="node.ourContributionAmount > 0">
              ...
              <span>
                ...
                <FormatQuenero :amount="node.ourContributionAmount" />
              </span>
            </span>
            ...
            <span v-if="node.awaitingContribution" class="contrib-amounts">
              ...
              {{ getMinContribution(node) }} QMR •
              {{ $t("strings.serviceNodeDetails.maxContribution") }}:
              {{ openForContriubtionQuenero(node) }} QMR
            </span>
          </q-item-label>
        </q-item-section>
        <q-item-section side>
        ...
        <ContextMenu
          ...
          @viewOnExplorer="openExplorer(node.masternode_pubkey)"
          @copyServiceNodeKey="copyKey(node.masternode_pubkey)"
        />
      </q-item>
    </q-list>
  </div>
